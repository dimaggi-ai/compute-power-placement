# Cover artwork — compute-power-placement

![A compute module on a sliding carriage between two power supply bases.](social-preview-v1.jpg)
...
Open [repository settings](https://github.com/dimaggi-ai/compute-power-placement/settings),
...
Alt text: A compute module on a sliding carriage between two power supply bases.
...
Subject: workload mobility toward available power. Two modest pale-metal platform bases sit a short distance apart on the chalk surface. One compact finned graphite compute cassette rests on a low sliding carriage between them, oriented toward the platform with a single robust copper power-feed connector. The other platform's connector is visibly empty. One short track connects the bases. Five or fewer main objects, no arrows, no trucks, no landscape, no batteries or colored indicator lights. The visual is a quiet study in where a workload can operate.
```
